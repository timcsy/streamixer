...
階段 1 至 6 已完成並發布 v1.2.0。Go 串流服務使用 fMP4（CMAF）分段格式、背景預生成、LRU+TTL 快取管理、API Key 認證、可設定 CORS。WordPress 外掛提供後台素材管理（CPT + Media Library）、Shortcode 與 Gutenberg Block 嵌入、響應式播放頁面、同步後自動清除本地檔案。Streamixer 以 Docker 獨立部署，WordPress 外掛獨立安裝。
...
**成功標準：**
- [x] 同步成功後 WordPress 端不再佔用音檔和圖片的儲存空間
- [x] 後台仍可正常顯示素材組合資訊（標題、檔案名稱、同步狀態）
- [x] 重新上傳檔案時流程正常（上傳→同步→清除）
- [x] 可透過設定頁關閉自動清除功能
...
交付：訪客可下載合成後的完整 MP4 影片檔，管理員可在後台批次匯出影片。
前置條件：階段 2.5

**前台下載（訪客）**：
- Streamixer 新增 `/download/{id}` 端點，即時將 fMP4 分段合併為完整 MP4 回傳
- WordPress 播放頁面與 Shortcode 播放器旁顯示「下載影片」按鈕
- 下載過程顯示進度或提示（長音檔合成需時間）

**後台匯出（管理員）**：
- WordPress 後台素材列表頁新增「匯出影片」批次操作
- 可選取多個素材組合，批次觸發下載或打包成 zip
- 匯出狀態顯示（進行中/完成/失敗）

**成功標準：**
- [ ] 訪客可從播放頁面下載完整 MP4 影片檔
- [ ] 下載的 MP4 可在本地播放器正常播放
- [ ] 管理員可在後台批次匯出多個素材的影片
- [ ] 長音檔（1 小時）的下載在合理時間內完成
